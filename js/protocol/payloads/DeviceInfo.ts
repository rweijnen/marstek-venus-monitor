...
export interface IDeviceInfo {
    ...
    dev_ver: string;       // Device version (153)
    bms_ver: string;       // BMS version (86)  
    fc_ver: string;        // FC version (202409090159P)
    [key: string]: string; // Allow for other key-value pairs
}

export class DeviceInfoPayload extends BasePayload {
    public parse(): IDeviceInfo {
    ...
    public toHTML(): string {
        try {
            ...
            if (data.type) html += `<div><strong>Device Type:</strong> ${data.type}</div>`;
            if (data.id) html += `<div><strong>Device ID:</strong> ${data.id}</div>`;
            if (data.sn) html += `<div><strong>Serial Number:</strong> ${data.sn}</div>`;
            if (data.mac) html += `<div><strong>MAC Address:</strong> ${data.mac}</div>`;
            if (data.fw) html += `<div><strong>Firmware Version:</strong> ${data.fw}</div>`;
            if (data.hw) html += `<div><strong>Hardware Version:</strong> ${data.hw}</div>`;
            if (data.dev_ver) html += `<div><strong>Device Version:</strong> ${data.dev_ver}</div>`;
            if (data.bms_ver) html += `<div><strong>BMS Version:</strong> ${data.bms_ver}</div>`;
            if (data.fc_ver) html += `<div><strong>FC Version:</strong> ${data.fc_ver}</div>`;
            ...
            return `<h3>📱 Device Information</h3><div class="error">Failed to parse device info: ${(error as Error).message}</div>`;
        }
    }
}