...
export interface IBMSData {
    bmsVersion?: number;
    voltageLimit?: string;
    chargeCurrentLimit?: string;
    dischargeCurrentLimit?: string;
...
export class BMSDataPayload extends BasePayload {
    public parse(): IBMSData {
        // Check device type (from global deviceType or determine from data)
        const deviceType = (window as any).uiController?.getDeviceType() || 'battery';
        
        if (deviceType === 'meter') {
            // Meter has minimal BMS data - just basic status
            if (this.payloadLength < 3) return {};
